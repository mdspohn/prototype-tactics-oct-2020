class MarkerRenderer extends Renderer {
    ...
    render(delta, ctx, camera, location, markers) {
        const RANGE_MARKER     = markers.getRange().get(location),
              PATH_MARKER      = markers.getPath().has(location),
        ...
        if (RANGE_MARKER !== undefined || SELECTION_MARKER !== undefined || FOCUS_MARKER) {
            const IS_SLOPE    = location.isSlope(),
                  IS_MIRRORED = IS_SLOPE && [Util.ORIENTATIONS.WEST, Util.ORIENTATIONS.EAST].includes(location.getOrientation()),
                  X_INDEX     = ~~IS_SLOPE * ([Util.ORIENTATIONS.WEST, Util.ORIENTATIONS.NORTH].includes(location.getOrientation()) ? 1 : 2),
